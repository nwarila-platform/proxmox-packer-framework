...
build {
  ...
  # SSH authentication is handled by ssh-agent on the runner using the keypair generated by
  # secure-packer-bootstrapper; no SSH password is wired through Ansible. ansible_become_pass
  # is still provided for sudo escalation and only lives on the runner's process list during
  # the build window. Consumer builds are expected to provide a real playbook path and an
  # ansible-playbook executable on PATH. The repository validation helpers inject a temporary
  # stub so framework syntax checks can run without bundling consumer Ansible content.
  provisioner "ansible" {
